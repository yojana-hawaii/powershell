function Invoke-fnGetInsuranceMismatchForValidation{
        
    $StoredProcedure = 'dbo.spGetInsuranceMismatchForValidation'
    $connection = New-spSqlConnection -StoredProcedureName $StoredProcedure
    $conn = $connection[0]
    $cmd = $connection[1]

     try{
        Write-Verbose -Message "Export insurance mismatch data for validation"
