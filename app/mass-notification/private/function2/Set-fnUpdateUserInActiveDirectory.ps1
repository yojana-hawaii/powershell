function Set-fnUpdateUserInActiveDirectory {
    [CmdletBinding()]
    param (
        [parameter()]
        [object]$users,
    ...
    foreach($user in $users){
        ...
        try {     
            ...
            if($manager.DistinguishedName  -eq $staff.manager){
                ...
                $managerUpdated += [PSCustomObject]@{
                    First = $user.'First Name'
                    Last = $user.'Last Name'
                    OldManager = $staff.Manager
                    NewManager = $user.ManagerEmail
                }
            }

            #region - Ask HR for Location, job title & department 
    ...
    return @($usersWithErrors, $managerUpdated)
